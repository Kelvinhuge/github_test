...
typedef struct
{
   UINT32 hertz; /**< Frequency in Hertz. */
   SINT32 level; /**< Power level in decibels where -63 means off. */
...
typedef struct
{
   UINT32 repetitions;     /**< Number of times to repeat frequencies in this tone. */
   UINT32 soundDuration;   /**< Sound duration in milliseconds. */
   UINT32 silenceDuration; /**< Silence duration in milliseconds. */
   UINT32 frequencies;     /**< Number of constituent frequencies. */
   BOOL   modulateTone;    /**< Tone modulation on/off. */
   UINT16 modulatePercent; /**< Percent of modulation. The modulation index is calculated by 32767 * (percent of modulation). */
...
typedef struct HDSP_TONE_PROV_PATTERN
{
   UINT32 repetitions;       /**< Number of iterations over the group. */
   UINT32 tones;             /**< Number of constituents. */
   BOOL noSteadyTone;        /**< Controls whether steady tone is played after tone has been completely played */
...
SINT16 hdspToneOff( UINT16 vhdHdl, HDSP_TONE_DIRECTION direction );
UINT16 hdspToneAdjustEgressPowerLevel( UINT16 powerLevel, int lineId );
SINT16 hdspDigitOff( UINT16 vhdHdl );
SINT16 hdspCmtdDBRInit( UINT16 vhdHdl, VRG_COUNTRY country );
